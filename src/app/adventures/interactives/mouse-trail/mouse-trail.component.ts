...
export class MouseTrailComponent implements OnInit {
  ...
  sample() {
    ...
    const rand = (min: number, max: number) => Math.floor(Math.random() * (max - min + 1)) + min,
      selectRandom = (items: any[]) => items[rand(0, items.length - 1)];

    const withUnit = (value: number, unit: string) => `${value}${unit}`,
      px = (value: number) => withUnit(value, 'px'),
    ...
    const appendElement = (element: HTMLElement) => document.body.appendChild(element),
      removeElement = (element: HTMLElement, delay: number) => setTimeout(() => document.body.removeChild(element), delay);
    const createStar = (position: any) => {
    ...
    const createGlowPoint = (position: any) => {
    ...
    const createGlow = (last: any, current: any) => {
    ...
    const updateLastStar = (position: any) => {
      last.starTimestamp = new Date().getTime();

      last.starPosition = position;
    };

    const updateLastMousePosition = (position: any) => last.mousePosition = position;

    const adjustLastMousePosition = (position: any) => {
